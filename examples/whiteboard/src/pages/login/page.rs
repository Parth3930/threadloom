        ...
        Section(row=false, height="screen", width="full", class="bg-background text-foreground flex flex-col items-center justify-center relative px-4") {
            ...
            Section(row=false, class="w-full max-w-sm border border-border rounded-xl bg-card shadow-sm p-8 flex flex-col gap-5") {
                ...
                Row(justify="center", class="mt-1 text-xs text-muted-foreground") {
                    Text(variant="span", class="text-xs text-muted-foreground") { "Don't have an account? " }
                    { threadloom_core::element("a").attr("href", "/signup").attr("class", "text-primary hover:underline ml-1 font-medium text-xs").child("Sign up").into_view() }
                }
            }
        }
    }
